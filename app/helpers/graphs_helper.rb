module GraphsHelper
	...
	def range(start_date, end_date)
		OfficeVisit.joins(:patient).joins(:parcels).where("(date(office_visits.created_at) >= ? AND date(office_visits.created_at) < ?) AND patients.user_id = ?", start_date, end_date, current_user.id).sum("parcels.value")
	end
